    ...
    <section id="compare" className="px-4 py-20">
      <div className="mx-auto max-w-5xl">
        <div className="mx-auto max-w-2xl text-center">
          <h2 className="font-display text-3xl font-bold text-slate-900 sm:text-4xl">A free tier that's actually usable</h2>
            ...
            <tbody className="divide-y divide-char-800">
              {ROWS.map((r) => (
                <tr key={r.name} className={r.us ? "bg-ember-500/5" : "bg-char-950"}>
                  ...
                  <td className="px-5 py-4 text-slate-700">{r.price}</td>
                  <td className="px-5 py-4 text-slate-700">{r.limit}</td>
                  <td className="px-5 py-4 text-slate-700">{r.ads}</td>
                  <td className="px-5 py-4 text-slate-700">{r.coach}</td>
                </tr>
              ))}
            </tbody>
          </table>
