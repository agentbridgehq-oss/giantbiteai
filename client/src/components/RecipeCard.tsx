  ...
  return (
    <article className="rounded-2xl border border-char-800 bg-char-900 p-6">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="whitespace-nowrap rounded-full bg-char-800 px-3 py-1 text-xs font-semibold capitalize text-gray-300">
          {r.difficulty}
        </span>
      </div>
      <p className="mt-2 text-sm text-gray-400">{r.summary}</p>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <ol className="mt-1.5 space-y-1.5 text-sm text-gray-400">
            {r.steps.map((s, i) => (
              <li key={i}>{i + 1}. {s}</li>
            ))}
          </ol>
      ...
      {r.substitutions.length > 0 && (
        <p className="mt-4 text-xs text-gray-500">
          <span className="font-semibold text-gray-400">Swaps: </span>
          {r.substitutions.join(" · ")}
        </p>
      )}
      <p className="mt-2 text-xs text-gray-500">
        <span className="font-semibold text-gray-400">Waste tip: </span>
        {r.wasteTip}
      </p>
